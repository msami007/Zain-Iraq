...
export default function AdminDeskWorkspace({
  ...
  // Workflow Routes state and fetch
  const [workflowRoutes, setWorkflowRoutes] = useState<any[]>([]);
  const [selectedWorkflowRouteId, setSelectedWorkflowRouteId] = useState<string>("");
  ...
  const [newWorkflowSteps, setNewWorkflowSteps] = useState<any[]>([]);

  const fetchWorkflowRoutes = async () => {
    try {
      const res = await fetch(`/api/v1/workflows?tenant_id=${tenantId}`);
      if (res.ok) {
  ...
  const startNewWorkflow = () => {
    setEditingWorkflow({ id: "new", name: "", description: "" });
    setNewWorkflowName("");
    setNewWorkflowDesc("");
    setNewWorkflowSteps([{ name: "Approval Step", role_restriction: "Admin", team_id: "", user_id: "" }]);
  };
  ...
  const startEditWorkflow = (route: any) => {
    ...
    setNewWorkflowSteps(route.steps.map((s: any) => ({
      name: s.name,
  ...
  const handleSaveWorkflow = async () => {
    ...
    for (let i = 0; i < newWorkflowSteps.length; i++) {
      if (!newWorkflowSteps[i].name.trim()) {
        alert(`Step ${i + 1} Name is required.`);
        return;
      }
    }
    ...
    try {
      ...
      if (isNew) {
        setWorkflowRoutes([...workflowRoutes, saved]);
      } else {
        setWorkflowRoutes(workflowRoutes.map(w => w.id === saved.id ? saved : w));
      }
  ...
  const handleDeleteWorkflow = async (routeId: string) => {
    if (!confirm("Are you sure you want to delete this workflow route? Associated articles will fallback to the default direct workflow.")) {
      return;
    }

    try {
      const res = await fetch(`/api/v1/workflows/${routeId}`, {
        method: "DELETE",
      });
      ...
      setWorkflowRoutes(workflowRoutes.filter(w => w.id !== routeId));
  ...
  const openEditor = async (article: AdminArticle) => {
    ...
    setSelectedWorkflowRouteId(article.workflow_route_id || "");
